...
module.exports = exports = function compare(orig, compareTo, tolerance, cb) {
    ...
    var diffFile = "";
    var command = 'gm compare -metric mse ' + orig + ' ' + compareTo;
    if ('function' == typeof tolerance) {
        cb = tolerance;
        tolerance = 0.4;
    }

    if ('object' == typeof tolerance) {
        diffFile = tolerance.file;
        tolerance = tolerance.tolerance || 0.4;
        command += ' -file ' + diffFile;
    }
